...
def main() -> int:
    ...
    try:
        ...
        loader = DataLoader(tokenized, batch_size=args.batch_size, sampler=sampler)
        ...
        for step in range(1, args.warmup_steps + args.steps + 1):
            ...
            output = ddp_model(input_ids=input_ids, attention_mask=attention_mask, labels=labels)
        ...
        dist.all_reduce(max_total_time, op=dist.ReduceOp.MAX)
        ...
        dist.all_gather_object(fingerprints, saved_fingerprint)
        parameters_synchronized = len(set(fingerprints)) == 1
        if rank == 0:
            args.checkpoint_dir.mkdir(parents=True, exist_ok=True)
            ddp_model.module.save_pretrained(args.checkpoint_dir, safe_serialization=True)
        ...
        if rank == 0:
            ...
            reload_verified = saved_fingerprint == fingerprint(reloaded) and bool(torch.isfinite(reload_loss).item())
